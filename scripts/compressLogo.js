...
// 需要压缩的logo文件
const logoConfigs = [
  { path: path.join(__dirname, '../src/assets/images/logo.png') }, // base包的logo
  { path: path.join(__dirname, '../src/assets/img/logo-3.png') } // 主包的logo-3
]
...
async function compressLogos() {
  for (const logoConfig of logoConfigs) {
    ...
    try {
      ...
      console.error(`压缩 ${path.basename(logoPath)} 失败:`, error.message)
    }
  }
